...
object TestDeployApp {
  ...
  def main(args: Array[String]): Unit = {
    ...
    /** Movies.json as args(0) GoodComedies.json as args(1)
      *
  ...
  /** How to run the Spark application on the Docker cluster:
    *
    *   1. Start the cluster (from the repo root) docker compose up --scale spark-worker=3
    *   2. Connect to the master node docker exec -it spark-master bash
    *   3. Run the spark-submit command /opt/spark/bin/spark-submit \ --class
    *      part6practical.TestDeployApp \ --master spark://spark-master:7077 \ --deploy-mode client
    *      \ --verbose \ /opt/spark-apps/spark-essentials_2.13-0.3.jar /opt/spark-apps/movies.json
    *      /opt/spark-apps/goodMovies
    */
}
